...
window.smoothScroll = {
	...
	smoothScrollTo: (endX, endY, dutation = smoothScroll.velocidade_padrao) => {
		...
		var header = 0;
		/* PC */
		dev.id('headerFixo') ? header = dev.id('headerFixo').clientHeight : false;

		/* Mobile*/
		dev.id('headerMobile') ? header = dev.id('headerMobile').clientHeight : false;
		...
		if(ancoraativa !== 2){
			...
			if(ancoraativa === 1){
				...
				if(dev.id(destino)){
					...
					var ydestino = dev.id(destino).offsetTop;
	...
	goTop: () => {
		if(ancoraativa !== 2){
			...
			if(ancoraativa === 1){
				...
				ancoraativa = 2;
				var ydestino = dev.id('push-loader').offsetTop;
