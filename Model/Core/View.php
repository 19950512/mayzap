...
class View {
	...
	public $header = array(
		...
		array('name' => 'description', 'content' => SITE_NOME),
	...
	private function layout($layout = 'Layout'){
		...
		$pes_nome = substr(($_SESSION[SESSION_LOGIN]['pes_nome'] ?? 'Conta'), 0, 15);
		...
		$mustache = array(
			...
			'{{ifloged}}' => (($_SESSION[SESSION_LOGIN]['app_id'] ?? '') ? 'conta' : 'login')
		);
	...
	public function getView($controlador = 'Index', $view = 'Index'){ 
		$pathView = DIR . DS . SUBPASTA . DS . VIEW . DS . $controlador . DS . $view . EXTENSAO_VIEW;
