...
class Pessoas extends Model{
	...
	**/
	...
	private function _getData($pes_email){

		// vis_ativo = 1 ATIVO
		$sql = $this->conexao->prepare('
			SELECT
				*
			FROM cad_pessoas AS pes
			LEFT JOIN cad_conta AS acc ON acc.acc_email = pes.pes_email
			WHERE pes.pes_email = :pes_email
		');
		...
		if($temp === false){     
			return ['r' => 'no', 'data' => 'Ops, a pessoa não foi encontrada.'];
		}
